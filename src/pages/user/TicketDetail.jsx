  ...
  const load = async () => {
    try {
      const [tRes, cRes, aRes, uRes] = await Promise.all([
        getTicket(id), getCommentsByTicket(id).catch(() => ({ data: { data: [] } })),
    ...
    <div className="max-w-3xl mx-auto">
      ...
      <Card className="mb-4">
        <div className="p-5">
          ...
          <p className="text-sm text-text-secondary mt-3 leading-relaxed">{ticket.description}</p>
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-y-3 gap-x-6 mt-5 text-xs">
            ...
            {ticket.resolutionNotes && <div className="col-span-2"><span className="text-text-muted">Resolution</span><p className="font-medium mt-0.5">{ticket.resolutionNotes}</p></div>}
            {ticket.rejectionReason && <div className="col-span-2"><span className="text-text-muted">Rejection Reason</span><p className="font-medium mt-0.5 text-danger">{ticket.rejectionReason}</p></div>}
          </div>
        </div>
      </Card>
      ...
      <div className="flex gap-1 mb-4 border-b border-border">
        {[
          ...
          { key: 'updates', label: 'Updates', icon: Clock, count: updates.length },
        ].map(t => (
              ...
              <div key={c.id} className="px-4 py-3">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    {c.edited && <span className="text-[10px] text-text-muted italic">(edited)</span>}
                  </div>
                  {c.commentOwnerUserId === user.userId && (
                    <button onClick={() => handleDeleteComment(c.id)} className="p-1 text-text-muted hover:text-danger"><Trash2 size={13} /></button>
                  )}
                </div>
      ...
      {tab === 'updates' && (
        <Card>
          {updates.length === 0 ? <p className="p-6 text-center text-sm text-text-muted">No technician updates</p> : (
            <div className="divide-y divide-border">
              {updates.map(u => (
